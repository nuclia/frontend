...
export class SynonymListComponent implements OnChanges, AfterViewInit {
  ...
  constructor(
    ...
    this.viewportRuler.change(200).pipe(takeUntil(this.unsubscribeAll)).subscribe(() => {
  ...
  updateVisibility() {
    ...
    items.forEach((item: ElementRef, index: number) => {
      const isVisible = index === 0 || item.nativeElement.getBoundingClientRect().right <= (containerPosition.right - COUNTER_WIDTH);
