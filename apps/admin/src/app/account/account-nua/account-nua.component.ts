  ...
  goToActivity(client: NUAClient): void {
    this.isNuaActivityEnabled
      .pipe(
        ...
        take(1),
        map((account) => this.navigation.getAccountManageUrl(account!.slug)),
      )
      .subscribe((accountManageUrl) =>
        this.router.navigateByUrl(`${accountManageUrl}/nua/${client.client_id}/activity`),
      );
  }
