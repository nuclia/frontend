...
export class KbListComponent implements OnInit, OnDestroy {
  ...
  manageKb(kb: IKnowledgeBoxItem): void {
    ...
    this.navigation.navigateExternal(this.navigation.getKbManageUrl(this.account?.slug || '', kb.slug || ''));
  }

  manageKbUsers(kb: IKnowledgeBoxItem): void {
    this.sdk.nuclia.options.zone = kb.zone;
    if (kb.role_on_kb) {
      this.navigation.navigateExternal(this.navigation.getKbUsersUrl(this.account!.slug, kb.slug!));
    } else {
  ...
  goToKb(kb: IKnowledgeBoxItem) {
    this.sdk.nuclia.options.zone = kb.zone;
    this.navigation.navigateExternal(this.navigation.getKbUrl(this.account?.slug || '', kb.slug || ''));
  }
