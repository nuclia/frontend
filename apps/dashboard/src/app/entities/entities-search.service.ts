...
export class EntitiesSearchService implements OnDestroy {
  ...
  private searchResults = combineLatest([
    this.runSearch,
    this.entitiesService.getEntities().pipe(filter((value): value is Entities => !!value)),
  ...
  hasNoResults(): Observable<boolean> {
    return this.searchResults.pipe(
      map(
        (results) => Object.keys(results).length > 0 && Object.keys(results).every((key) => results[key].length === 0),
      ),
    );
  }

  getSearchResults(groupKey: string): Observable<string[] | null> {
    return this.searchResults.pipe(map((groups) => groups[groupKey] || null));
  }
