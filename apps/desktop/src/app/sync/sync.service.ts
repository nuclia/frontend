...
export class SyncService {
  ...
  destinations: { [id: string]: IDestinationConnector } = {
    nucliacloud: new NucliaCloudKB(this.nuclia),
    algolia: new Algolia(),
  };
                ...
                sync.files
                  ...
                  .map((f) =>
                    source.download(f).pipe(
                      concatMap((blob) => {
                        if (destination.id === 'nucliacloud') {
                          return destination.upload(f.title, blob).pipe(tap(() => (f.status = FileStatus.UPLOADED)));
                        } else {
                          return this.nuclia.db.upload(new File([blob], f.title)).pipe(
                            tap((token) => {
                              console.log(token);
