...
export class KeyValueFieldComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    const existingValue = this.keyValueGroup.value;
    ...
    this.keyValueGroup.valueChanges.pipe(takeUntil(this.destroy$), debounceTime(300)).subscribe(() => {
  ...
  removeKeyValue(controlKey: string) {
    this.keyValueGroup.removeControl(controlKey);
  ...
  /**
   * Synchronize the key-value pairs to the parent form as a flat object
   */
  private syncToParentForm() {
    const objectValue = this.getObjectValue();

    // Clear existing controls in the parent form control group
    const parentControl = this.form.get(this.controlName) as FormGroup;
    if (parentControl) {
      // Clear all existing dynamic controls but keep our key-value structure
      Object.keys(parentControl.controls).forEach((key) => {
        if (!key.startsWith('keyValue')) {
          parentControl.removeControl(key);
        }
      });

      // Add new controls for each key-value pair
      Object.entries(objectValue).forEach(([key, value]) => {
        if (key && value !== undefined) {
          parentControl.addControl(key, new FormControl(value));
        }
      });
    }
  }
  ...
  getObjectValue(): Record<string, any> {
    ...
    Object.values(this.keyValueGroup.controls).forEach((control) => {
