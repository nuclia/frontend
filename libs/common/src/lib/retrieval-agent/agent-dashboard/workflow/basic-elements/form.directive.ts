...
export abstract class FormDirective {
  ...
  submit() {
    if (this.form.valid) {
      ...
      // Convert FormArray values back to comma-separated strings for multiselect fields
      const processedValue = { ...rawValue };

      for (const [key, value] of Object.entries(rawValue)) {
        if (Array.isArray(value)) {
          // Check if this field is configured as multiselect
          const control = this.configForm.get(key);
          if (control instanceof FormArray) {
            // Convert array to comma-separated string, filtering out empty values
            processedValue[key] = value.filter((v: any) => v && String(v).trim() !== '').join(',');
          }
        }
      }
