...
const AUTO_FILE_FIELD_ID = /^[a-z0-9]{32}$/;
...
export class EditResourceComponent implements OnInit, OnDestroy {
  ...
  originalFields: Observable<(ResourceFieldWithIcon & { isAutogeneratedFileFieldId: boolean })[]> = this.fields.pipe(
    map((fields) =>
      fields
        .filter((field) => !field.field_id.startsWith('da-') && !isMemoryFieldId(field.field_id))
        .map((field) => ({ ...field, isAutogeneratedFileFieldId: AUTO_FILE_FIELD_ID.test(field.field_id) })),
    ),
  );
