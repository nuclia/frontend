...
import { CHAT_ACTIVITY_SHOW_FIELDS } from './chat-activity-page.config';
import { AbstractMetricsPageService } from '../abstract-metrics-page.service';

@Injectable()
export class ChatActivityPageService extends AbstractMetricsPageService<ActivityLogItem> {
  ...
  // Sidebar numeric/date/boolean filter state
  ...
  private _buildFilters(): ActivityLogChatFilters {
    ...
    return filters as ActivityLogChatFilters;
  }
  ...
  protected _queryPage(isAppend: boolean): Observable<ActivityLogItem[]> {
    ...
    return this.sdk.currentKb.pipe(
      take(1),
      switchMap((kb) =>
        kb.activityMonitor.queryActivityLogs(EventType.CHAT, {
          year_month: this._yearMonth(),
          show: CHAT_ACTIVITY_SHOW_FIELDS,
