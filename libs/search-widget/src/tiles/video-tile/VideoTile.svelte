<script lang="ts">
  ...
  const playFrom = (time: number, selectedParagraph?: Search.Paragraph) => {
    ...
    const paragraph$ =
      selectedParagraph && isFileOrLink(selectedParagraph.field_type)
        ? of(selectedParagraph)
        : matchingParagraphs.pipe(
            map((paragraphList) => paragraphList.filter((p) => isFileOrLink(p.field_type))[0] || paragraphList[0]),
          );
    if (!resource) {
