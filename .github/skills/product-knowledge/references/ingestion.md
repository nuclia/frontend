...
Structured data stored as key-value pairs conforming to a predefined **schema** — typed and validated, well-suited for metadata that needs to be filtered/searched programmatically (product attributes, document properties, etc.). Unlike other field types, KV fields are not generated automatically by the processor; they must be written via the API (`/docs/ingestion/how-to/use-kv-fields`) or generated by a **Generator** Data Augmentation agent.

**Schemas:**

- Scoped to a Knowledge Box, managed via `/kb/{kbid}/kv-schemas`.
- Limits: up to **20 schemas** per KB, each with up to **50 fields**.
- Each schema has `id`, optional `description` (used by LLM agents to understand the field's purpose), and a `fields` list.
- Each field has `key`, `type`, optional `description` (also used by agents), and is `required` by default (set `required: false` to make optional).

**Field types:** `text`, `integer`, `float`, `boolean`, `date` (RFC 3339 string, e.g. `"2024-01-15T00:00:00Z"`).

- **Range mode** (`"range": true`): available on `integer`, `float`, `date` — stores a closed interval (`{"lower": ..., "upper": ...}`) instead of a scalar. Enables `contains` filters (value falls within the interval).
- **Repeated mode** (`"repeated": true`): available on `text` — stores a list of strings instead of a single one. Enables `contains` filters.

**Writing data:**

- Inline when creating/updating a resource: `POST /kb/{kbid}/resources` with a `key_values` map keyed by schema `id`.
- Directly on an existing resource: `PUT /kb/{kbid}/resource/{rid}/key_value/{field_id}`.
- Data is validated against the schema at write time — unknown keys, missing required keys, or wrong types return `422`.

**Querying:** filter on KV fields via `filter_expression` (see [Key-Value filters](./search-and-rag.md#key-value-filters) — `eq`, `gte`/`lte`, `contains`).
